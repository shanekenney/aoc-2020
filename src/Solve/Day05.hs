...
toRowMove :: Char -> Maybe RowMove
toRowMove = \case
  'F' -> Just MoveFront
  'B' -> Just MoveBack
  _ -> Nothing

toColMove :: Char -> Maybe ColMove
toColMove = \case
  'L' -> Just MoveLeft
  'R' -> Just MoveRight
  _ -> Nothing
...
readBoardingPass str =
  BoardingPass
    { rowMoves = fromJust . toRowMove <$> rowStr,
      colMoves = fromJust . toColMove <$> colStr
    }
  where
    [rowStr, colStr] = splitPlaces ([7, 3] :: [Int]) str

getRow :: BoardingPass -> Int
getRow BoardingPass {rowMoves} =
  fst $ foldl findRow initial rowMoves
  where
    initial = (0, 127)
    findRow (lower, upper) =
...
getCol :: BoardingPass -> Int
getCol BoardingPass {colMoves} =
  fst $ foldl findCol initial colMoves
  where
    initial = (0, 7)
    findCol (lower, upper) =
      let mid = fromIntegral (lower + upper) / 2 :: Double
...
getSeatId boardingPass = row * 8 + col
  where
